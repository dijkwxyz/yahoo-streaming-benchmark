...
#    cp conf/kafka/* $KAFKA_DIR/config/
  ...
  then
#    cd data
    start_if_needed KafkaDataGenerator "Load Generation" 1 java -cp $BASE_DIR/flink-benchmarks/target/flink-benchmarks-0.1.0.jar flink.benchmark.generator.KafkaDataGenerator $BASE_DIR/$CONF_FILE
...
#    stop_if_needed leiningen.core.main "Load Generation"
  ...
  then
    ...
    remote_operation_sync $FLINK_HOST "STOP_FLINK"
    for ((num=1; num <=$KAFKA_HOST_NUM; num++)); do
      remote_operation_sync $KAFKA_HOST_PREFIX$num "STOP_KAFKA"
    done
